...
Stock is global per ingredient (not partitioned by `location_id`). `location_id` is recorded on each order for traceability only.
...
Seeded net stock for `BRS-BEEF-001`: `50 + 30 − 15 − 5 = 60` kg.
