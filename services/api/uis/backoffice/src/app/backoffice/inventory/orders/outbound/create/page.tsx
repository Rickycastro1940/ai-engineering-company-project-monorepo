...
export default function InventoryOutboundOrderCreatePage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSuccessMessage('Outbound Order submitted successfully.');
      setSelectedProductId('');
      setSelectedProductStock(null);
      setQuantity('');
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to submit Outbound Order';
        ...
        <>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '12px', marginTop: '16px' }}>
            ...
            <button type="submit" disabled={isLoadingProducts || isSubmitting || products.length === 0 || !selectedProductId || isLoadingSelectedStock}>
              {isSubmitting ? 'Submitting...' : 'Submit Outbound Order'}
            </button>
          </form>
