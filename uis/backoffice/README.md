...
Protected staff views (all of them): `/`, `/accessible`, `/inventory`, `/inventory/products`, `/inventory/orders`, `/inventory/orders/inbound`, `/inventory/orders/outbound`, `/inventory/orders/new`, `/account/profile`, `/account/change-password`, and any unmatched path (`*`). Public in this app: `/login`, `/register`. `/backoffice/inventory/orders` redirects to `/inventory/orders`.
