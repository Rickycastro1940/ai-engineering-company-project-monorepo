...
try:
    from data.pipelines.job_runs import JobRun, JobRunStore, execute_with_job_tracking, get_job_run_store
except ImportError:  # pragma: no cover - direct script execution
    from job_runs import JobRun, JobRunStore, execute_with_job_tracking, get_job_run_store
...
supabase: Optional["Client"] = create_client(url, key) if create_client and url and key else None
...
def _execute_etl(start_date: str, end_date: str) -> int:
...
def run_pipeline(
    ...
    """Run the weekly pipeline; lifecycle is persisted only in job_runs."""
    job_store = store or get_job_run_store(supabase)
    resolved_target = target_date or start_date

    return execute_with_job_tracking(
        store=job_store,
        target_date=resolved_target,
        execute_fn=lambda: _execute_etl(start_date, end_date),
    )


if __name__ == "__main__":
    print("Executing pipeline directly...")
    run_pipeline("2026-07-01", "2026-08-01", target_date="2026-07-01")
